<cfcomponent displayname="BuildingGateway" output="NO"
   hint="I am the BuildingGateway Class">
   ...
   <cfset variables.instance.datasource = '' />
    ...
    <cffunction name="getEveryBuilding" access="PUBLIC" output="NO"
       ...
       <cfreturn filterEveryBuilding() />
     </cffunction>
     
     <cffunction name="filterByName" access="PUBLIC" output="NO"
         hint="I run a query of every Building within the database table based upon a 
            required filter  -- Building Name ">
         <cfargument name="nameFilter" required="YES" type="STRING" 
             hint="I am the building name filter" /> 
             ...
             <!--- Send the structure into the query method and return the query object --->
                <cfreturn filterEveryBuilding(filter) />
     </cffunction>   
                                         
         <!--- PRIVATE METHODS --->
         <cffunction name="filterEveryBuilding" access="PRIVATE" output="NO"
          hint="I run a query and will return all Building records. If a filter 
             ...
             <cfset var qSearch = '' />
                ...
                <cfif NOT structIsEmpty(arguments.filter)>
                   <!--- A filter has been provided. Let's find out which filter it is, 
                      ...
                      <!--- Perform an exact match on the building name  --->
