<cfcomponent displayname="GenderGateway" output="NO"
   hint="I am the GenderGateway Class">
   ...
   <cfset variables.instance.datasource = '' />
    ...
    <cffunction name="getEveryGender" access="PUBLIC" output="NO"
       ...
       <cfreturn filterEveryGender() />
     </cffunction>
     
     <cffunction name="filterByName" access="PUBLIC" output="NO"
         hint="I run a query of every Gender within the database table based upon a 
            required filter  -- Gender Name ">
         <cfargument name="nameFilter" required="YES" type="STRING" 
             hint="I am the gender name filter" /> 
             ...
             <!--- Send the structure into the query method and return the query object --->
                <cfreturn filterEveryGender(filter) />
             </cffunction>   
                                         
         <!--- PRIVATE METHODS --->
         <cffunction name="filterEveryGender" access="PRIVATE" output="NO"
          hint="I run a query and will return all Gender records. If a filter 
             ...
             <cfset var qSearch = '' />
                ...
                <cfif NOT structIsEmpty(arguments.filter)>
                   <!--- A filter has been provided. Let's find out which filter it is, 
                      ...
                      <!--- Perform an exact match on the gender name  --->
