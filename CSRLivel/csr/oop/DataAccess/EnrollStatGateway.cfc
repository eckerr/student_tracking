<cfcomponent displayname="EnrollStatGateway" output="NO"
   hint="I am the EnrollStatGateway Class">
   ...
   <cfset variables.instance.datasource = '' />
    ...
    <cffunction name="getEveryEnrollStat" access="PUBLIC" output="NO"
       ...
       <cfreturn filterEveryEnrollStat() />
     </cffunction>
     
     <cffunction name="filterByName" access="PUBLIC" output="NO"
         hint="I run a query of every EnrollStat within the database table based upon a 
            required filter  -- EnrollStat Name ">
         <cfargument name="nameFilter" required="YES" type="STRING" 
             hint="I am the enrollStat name filter" /> 
             ...
             <!--- Send the structure into the query method and return the query object --->
                <cfreturn filterEveryEnrollStat(filter) />
             </cffunction>   
                                         
         <!--- PRIVATE METHODS --->
         <cffunction name="filterEveryEnrollStat" access="PRIVATE" output="NO"
          hint="I run a query and will return all EnrollStat records. If a filter 
             ...
             <cfset var qSearch = '' />
                ...
                <cfif NOT structIsEmpty(arguments.filter)>
                   <!--- A filter has been provided. Let's find out which filter it is, 
                      ...
                      <!--- Perform an exact match on the enrollStat name  --->
