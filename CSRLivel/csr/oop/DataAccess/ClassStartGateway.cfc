<cfcomponent displayname="ClassStartGateway" output="NO"
   hint="I am the ClassStartGateway Class">
   ...
   <cfset variables.instance.datasource = '' />
    ...
    <cffunction name="getEveryClassStart" access="PUBLIC" output="NO"
       ...
       <cfreturn filterEveryClassStart() />
     </cffunction>
     
     <cffunction name="filterByName" access="PUBLIC" output="NO"
         hint="I run a query of every ClassStart within the database table based upon a 
            required filter  -- ClassStart Name ">
         <cfargument name="nameFilter" required="YES" type="STRING" 
             hint="I am the classStart name filter" /> 
             ...
             <!--- Send the structure into the query method and return the query object --->
                <cfreturn filterEveryClassStart(filter) />
     </cffunction>   
                                         
         <!--- PRIVATE METHODS --->
         <cffunction name="filterEveryClassStart" access="PRIVATE" output="NO"
          hint="I run a query and will return all ClassStart records. If a filter 
             ...
             <cfset var qSearch = '' />
                ...
                <cfif NOT structIsEmpty(arguments.filter)>
                   <!--- A filter has been provided. Let's find out which filter it is, 
                      ...
                      <!--- Perform an exact match on the classStart name  --->
