<cfcomponent displayname="StartTimeGateway" output="NO"
   hint="I am the StartTimeGateway Class">
   ...
   <cfset variables.instance.datasource = '' />
    ...
    <cffunction name="getEveryStartTime" access="PUBLIC" output="NO"
       ...
       <cfreturn filterEveryStartTime() />
     </cffunction>
     
     <cffunction name="filterByName" access="PUBLIC" output="NO"
         hint="I run a query of every StartTime within the database table based upon a 
            required filter  -- StartTime Name ">
         <cfargument name="nameFilter" required="YES" type="STRING" 
             hint="I am the startTime name filter" /> 
             ...
             <!--- Send the structure into the query method and return the query object --->
                <cfreturn filterEveryStartTime(filter) />
     </cffunction>   
                                         
         <!--- PRIVATE METHODS --->
         <cffunction name="filterEveryStartTime" access="PRIVATE" output="NO"
          hint="I run a query and will return all StartTime records. If a filter 
             ...
             <cfset var qSearch = '' />
                ...
                <cfif NOT structIsEmpty(arguments.filter)>
                   <!--- A filter has been provided. Let's find out which filter it is, 
                      ...
                      <!--- Perform an exact match on the startTime name  --->
