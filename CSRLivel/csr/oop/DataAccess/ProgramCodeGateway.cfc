<cfcomponent displayname="ProgramCodeGateway" output="NO"
   hint="I am the ProgramCodeGateway Class">
   ...
   <cfset variables.instance.datasource = '' />
    ...
    <cffunction name="getEveryProgramCode" access="PUBLIC" output="NO"
       ...
       <cfreturn filterEveryProgramCode() />
     </cffunction>
     
     <cffunction name="filterByName" access="PUBLIC" output="NO"
         hint="I run a query of every ProgramCode within the database table based upon a 
            required filter  -- ProgramCode Name ">
         <cfargument name="nameFilter" required="YES" type="STRING" 
             hint="I am the programCode name filter" /> 
             ...
             <!--- Send the structure into the query method and return the query object --->
                <cfreturn filterEveryProgramCode(filter) />
     </cffunction>   
                                         
         <!--- PRIVATE METHODS --->
         <cffunction name="filterEveryProgramCode" access="PRIVATE" output="NO"
          hint="I run a query and will return all ProgramCode records. If a filter 
             ...
             <cfset var qSearch = '' />
                ...
                <cfif NOT structIsEmpty(arguments.filter)>
                   <!--- A filter has been provided. Let's find out which filter it is, 
                      ...
                      <!--- Perform an exact match on the programCode name  --->
