<cfcomponent displayname="EndTimeGateway" output="NO"
   hint="I am the EndTimeGateway Class">
   ...
   <cfset variables.instance.datasource = '' />
    ...
    <cffunction name="getEveryEndTime" access="PUBLIC" output="NO"
       ...
       <cfreturn filterEveryEndTime() />
     </cffunction>
     
     <cffunction name="filterByName" access="PUBLIC" output="NO"
         hint="I run a query of every EndTime within the database table based upon a 
            required filter  -- EndTime Name ">
         <cfargument name="nameFilter" required="YES" type="STRING" 
             hint="I am the endTime name filter" /> 
             ...
             <!--- Send the structure into the query method and return the query object --->
                <cfreturn filterEveryEndTime(filter) />
     </cffunction>   
                                         
         <!--- PRIVATE METHODS --->
         <cffunction name="filterEveryEndTime" access="PRIVATE" output="NO"
          hint="I run a query and will return all EndTime records. If a filter 
             ...
             <cfset var qSearch = '' />
                ...
                <cfif NOT structIsEmpty(arguments.filter)>
                   <!--- A filter has been provided. Let's find out which filter it is, 
                      ...
                      <!--- Perform an exact match on the endTime name  --->
