<cfcomponent displayname="DaysGateway" output="NO"
   hint="I am the DaysGateway Class">
   ...
   <cfset variables.instance.datasource = '' />
    ...
    <cffunction name="getEveryDays" access="PUBLIC" output="NO"
       ...
       <cfreturn filterEveryDays() />
     </cffunction>
     
     <cffunction name="filterByName" access="PUBLIC" output="NO"
         hint="I run a query of every Days within the database table based upon a 
            required filter  -- Days Name ">
         <cfargument name="nameFilter" required="YES" type="STRING" 
             hint="I am the days name filter" /> 
             ...
             <!--- Send the structure into the query method and return the query object --->
                <cfreturn filterEveryDays(filter) />
     </cffunction>   
                                         
         <!--- PRIVATE METHODS --->
         <cffunction name="filterEveryDays" access="PRIVATE" output="NO"
          hint="I run a query and will return all Days records. If a filter 
             ...
             <cfset var qSearch = '' />
                ...
                <cfif NOT structIsEmpty(arguments.filter)>
                   <!--- A filter has been provided. Let's find out which filter it is, 
                      ...
                      <!--- Perform an exact match on the days name  --->
