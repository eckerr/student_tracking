<cfcomponent displayname="ShiftDescripGateway" output="NO"
   hint="I am the ShiftDescripGateway Class">
   ...
   <cfset variables.instance.datasource = '' />
    ...
    <cffunction name="getEveryShiftDescrip" access="PUBLIC" output="NO"
       ...
       <cfreturn filterEveryShiftDescrip() />
     </cffunction>
     
     <cffunction name="filterByName" access="PUBLIC" output="NO"
         hint="I run a query of every ShiftDescrip within the database table based upon a 
            required filter  -- ShiftDescrip Name ">
         <cfargument name="nameFilter" required="YES" type="STRING" 
             hint="I am the shiftDescrip name filter" /> 
             ...
             <!--- Send the structure into the query method and return the query object --->
                <cfreturn filterEveryShiftDescrip(filter) />
     </cffunction>   
                                         
         <!--- PRIVATE METHODS --->
         <cffunction name="filterEveryShiftDescrip" access="PRIVATE" output="NO"
          hint="I run a query and will return all ShiftDescrip records. If a filter 
             ...
             <cfset var qSearch = '' />
                ...
                <cfif NOT structIsEmpty(arguments.filter)>
                   <!--- A filter has been provided. Let's find out which filter it is, 
                      ...
                      <!--- Perform an exact match on the shiftDescrip name  --->
