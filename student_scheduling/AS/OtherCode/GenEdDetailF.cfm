...
<CFOUTPUT QUERY="stuStuff">
       <!--- Temporary filter to warn of incorrect data--->
       <CFSET BadMajID= #MajorID#> 
		<!--------------end filter------------------------->
 <CFSET GenNeedF= #GenNeed#>
 <CFIF #M2L# EQ 0>
 	<CFSET M2LND = 1>
    <CFELSE>
      <CFSET M2LND = 0>
...
<CFSET GECountMatch = (#GenNeedF#)- ((GEclasses.RecordCount)+(#M2LND#+#H2LND#+#F2LND#+#S2LND#+#L3LND#+#F3LND#+#S3LND#))>
...
<TABLE BORDER="0" CELLPADDING = "5">
  ...
  <CFIF #GECountMatch# NEQ 0>
       <B><FONT color=Red> Gen Ed class mismatch -- see original audit</FONT></B>
        ...
        <CFIF #BadMajID# EQ "BAP " >
           <B><FONT color=Red> BAP DATA IS INCORRECT - see original audit</FONT></B> 
       </CFIF>
         <CFIF #BadMajID# EQ "BDPH" >
           <B><FONT color=Red> BDPH DATA IS INCORRECT - see original audit</FONT></B> 
       </CFIF>
         <CFIF #BadMajID# EQ "VFX " >
           <B><FONT color=Red> VFX DATA IS INCORRECT - see original audit</FONT></B> 
       </CFIF>
         <CFIF #BadMajID# EQ "DFVP" >
           <B><FONT color=Red> DFVP DATA IS INCORRECT - see original audit</FONT></B> 
       </CFIF> 
        <CFIF #BadMajID# EQ "DWIM" >
           <B><FONT color=Red> DWIM DATA IS INCORRECT - see original audit</FONT></B> 
       </CFIF>
   </cfoutput>   
